...
export default function AddDoctorScreen({ navigation }) {
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.specialization || !form.email || !form.phone) {
      return Alert.alert('Validation', 'Name, specialization, email and phone are required');
    }
    setLoading(true);
    try {
      ...
      if (image) {
        formData.append('image', {
          uri: image.uri, name: 'doctor.jpg', type: 'image/jpeg',
        });
      }
      ...
      Alert.alert('Error', err.response?.data?.message || 'Failed to add doctor');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitText}>Save Doctor</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
}
